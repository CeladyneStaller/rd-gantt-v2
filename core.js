...
(function (root) {
  ...
  function cascade(portfolio, execDocs, today) {
    ...
    function pfGateEarliest(s) { return (s.actualDate != null) ? s.actualDate : today; }
    function computeGateEarliest(gid) {
      ...
      var seed = pfGateEarliest(g);
      if (g.actualDate != null) { gateEarliest[gid] = seed; return seed; }
      if (gateEStack[gid]) { gateEarliest[gid] = seed; return seed; }
      gateEStack[gid] = true;
      var res = seed, preds = gatePreds[gid];
      for (var i = 0; i < preds.length; i++) {
        if (gateEStack[preds[i].from]) continue;
        res = Math.max(res, computeGateEarliest(preds[i].from) + preds[i].lag);
      }
    ...
    function computeProjEndEarliest(id) {
      ...
      var c = childCache[id]; var hasKids = c && (c.sgs.length + c.tasks.length) > 0;
      // an objective whose work IS its gates/tasks can finish as early as they allow (floored at its earliest start);
      // a childless objective still occupies its planned duration, so it shows no spurious acceleration
      var res = hasKids ? Math.max(intrinsicEarliest[id], cs) : Math.max(intrinsicEarliest[id], cs + (o.plannedEnd - o.plannedStart));
      objEStack[id] = false;
