...
(function (root) {
  ...
  function cascade(portfolio, execDocs, today) {
    ...
    // Both are additive; with neither, a gate has no preds and gateEff == pfGate.
    var gatePreds = {};
    Object.keys(gateById).forEach(function (gid) { gatePreds[gid] = []; });
    (portfolio.stageGateEdges || []).forEach(function (e) {
      if (gatePreds[e.toGate]) gatePreds[e.toGate].push({ from: e.fromGate, lag: e.lagDays || 0 });
    });
    objs.forEach(function (o) {
      if (!o.chainGatesByDate) return;
