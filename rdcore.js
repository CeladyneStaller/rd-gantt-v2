...
(function (root) {
  ...
  function progressLinear(value, target, direction) {
    ...
    if (direction === 'down') {
      // 'down' is scored by the ratio target/value, which needs a guard at zero because it divides by
      // the measurement. That guard used to return 0 for a measured zero — the WORST score for the BEST
      // possible result. It bit hardest on spread statistics, where zero is the ideal: three identical
      // samples give CV = 0, i.e. perfect repeatability, and scored 0. Stating the rule directly instead
      // of patching the singularity also fixes the sign cases the ratio never handled.
      if (value <= target) return 100;      // met or beaten (covers zero, and negatives on both sides)
      if (value <= 0) return 0;             // worse than target and non-positive: the ratio is meaningless
      return 100 * target / value;          // worse than target: partial credit by ratio
    }
    return 0;
  }
  ...
  // An unscored Key Result counts as ZERO, not as absent — the same stance meanScorable takes for an
  ...
  /* ---- Gantt timeframe: a set of selected quarters -------------------------------------------------
     The selector is a grid of years x quarters and clicking TOGGLES a cell, so the selection can end up
     non-contiguous: 2026Q1 and 2026Q4 with nothing between. A Gantt axis has to be one continuous span,
     so the window is earliest-selected to latest-selected and the quarters in between are IMPLIED — the
     UI shades them differently so the grid never claims a gap it cannot honour.
     An empty selection is not an error state: it falls back to the current quarter. */
  function qKey(y, n){ return String(y) + 'Q' + String(n); }
  function qParse(q){
    var m = /^(\d{4})Q([1-4])$/.exec(String(q == null ? '' : q).trim());
    return m ? { y: +m[1], n: +m[2] } : null;
  }
  function qIndex(q){ var p = qParse(q); return p ? (p.y * 4 + (p.n - 1)) : null; }
  function qFromIndex(i){ return qKey(Math.floor(i / 4), (i % 4) + 1); }

  // the full inclusive run between the earliest and latest selected quarter
  function quarterSpan(sel){
    var idx = (sel || []).map(qIndex).filter(function (i) { return i != null; });
    if (!idx.length) return [];
    var lo = Math.min.apply(null, idx), hi = Math.max.apply(null, idx), out = [];
    for (var i = lo; i <= hi; i++) out.push(qFromIndex(i));
    return out;
  }
  // which of those the user did NOT click — the implied middle
  function quarterSpanImplied(sel){
    var have = {}; (sel || []).forEach(function (q) { if (qIndex(q) != null) have[q] = 1; });
    return quarterSpan(sel).filter(function (q) { return !have[q]; });
  }
  // the day window the axis should show, or null when nothing is selected
  function quarterSpanRange(sel){
    var span = quarterSpan(sel);
    if (!span.length) return null;
    var a = quarterRange(span[0]), b = quarterRange(span[span.length - 1]);
    if (!a || !b) return null;
    return { from: span[0], to: span[span.length - 1], start: a.start, end: b.end };
  }
  /* Toggle: clicking a selected quarter removes it, clicking an unselected one adds it. An IMPLIED
     quarter is not in the set, so clicking it adds it — the window does not move, which is the point. */
  function toggleQuarter(sel, q){
    if (qIndex(q) == null) return (sel || []).slice();
    var out = [], hit = false;
    (sel || []).forEach(function (x) { if (x === q) hit = true; else out.push(x); });
    if (!hit) out.push(q);
    return out.sort(function (a, b) { return qIndex(a) - qIndex(b); });
  }
  // every quarter of a year, for the year button
  function yearQuarters(y){ return [1, 2, 3, 4].map(function (n) { return qKey(y, n); }); }
  /* A year click is the same toggle applied to four cells at once: if the year is ALREADY fully
     selected it clears, otherwise it fills. Anything else would make the year button's effect depend on
     which quarters happened to be on, which is unpredictable to use. */
  function toggleYear(sel, y){
    var qs = yearQuarters(y), have = {};
    (sel || []).forEach(function (x) { have[x] = 1; });
    var full = qs.every(function (q) { return have[q]; });
    var out = (sel || []).filter(function (x) { return qs.indexOf(x) < 0; });
    if (!full) out = out.concat(qs);
    return out.sort(function (a, b) { return qIndex(a) - qIndex(b); });
  }
  ...
  function cascade(portfolio, execDocs, today) {
    ...
    /* A gate re-committed by an active catch-up plan is a deliberate new commitment: the team has said
       "we will hit THIS date regardless of what happened upstream". Chained gates take their lag from
       the ORIGINAL planned spacing, so without this a single late predecessor pushes every downstream
       gate by the same number of days forever and the re-commitment changes nothing — the schedule card
       keeps reporting the original overrun and keeps demanding a plan that already exists.
       This is the same exemption `locked` already provides, for the same reason. The seed still floors
       at today via pfGate, so a re-committed date in the past cannot read as on-time. */
    function gateRecommitted(gid) {
      var o = gateObjOf[gid]; if (!o) return false;
      return !!catchupEntry(execFor(o.divisionId).catchupPlans, o.id, gid);
    }
    function computeGateEff(gid) {
      ...
      if (!g) return 0;
      var seed = pfGate(g);
      if (g.locked || gateRecommitted(gid)) { gateEff[gid] = seed; return seed; }
  ...
  // An objective used to hold exactly one experiment tree. It now holds several INDEPENDENT trees:
  // each owns its own `experiments` map, so an id in one tree is invisible to another and
  // reachability physically cannot cross between them. Schema 1 (a bare tree object) is wrapped as a
  // single tree named "Main" on read, so old documents keep working and are rewritten on next save.
  //
  // Unknown fields are carried through untouched. migrateProblem's field whitelist silently discarded
  // FMEA evidence on every load and cost several sessions to find; this normaliser must never repeat it.
  function etbTreesOf(bag) {
    if (!bag || typeof bag !== 'object') return { trees: [], activeTid: null, schema: 2 };
    if (Array.isArray(bag.trees)) {
      var ts = bag.trees.filter(function (t) { return t && typeof t === 'object'; });
      var active = bag.activeTid && ts.some(function (t) { return t.tid === bag.activeTid; })
        ? bag.activeTid : (ts.length ? ts[0].tid : null);
      return { trees: ts, activeTid: active, schema: 2 };
    }
    // schema 1: the bag IS the tree. Preserve every field; only add the wrapper.
    var one = Object.assign({}, bag);
    if (!one.tid) one.tid = 't_main';
    if (!one.name) one.name = 'Main';
    return { trees: [one], activeTid: one.tid, schema: 1 };
  }
  function etbTreeById(bag, tid) {
    var w = etbTreesOf(bag);
    for (var i = 0; i < w.trees.length; i++) if (w.trees[i].tid === tid) return w.trees[i];
    return null;
  }
  function etbActiveTree(bag) {
    var w = etbTreesOf(bag);
    return etbTreeById(bag, w.activeTid) || w.trees[0] || null;
  }

  // How many experiments still lie ahead of `expId`, shortest and longest path to termination.
  //   - a result whose branch ends in a terminal (FMEA, halt, target achieved) contributes 0
  //   - a result targeting experiment E contributes 1 + depth(E)
  //   - max is the longest such path, min the shortest
  // `unplanned` flags a result with neither a target nor a terminal: it counts as 0, but it means
  // "not thought through yet" rather than "ends here", and the UI should say so.
  // Cycles are possible via repeat_of, so the walk is depth-capped and reports `capped`.
  var ETB_DEPTH_CAP = 8;
  function nextStepRange(tree, expId) {
    var exps = (tree && tree.experiments) || {};
    var capped = false, sawUnplanned = false;
    function walk(id, depth, seen) {
      var e = exps[id];
      if (!e) return { min: 0, max: 0 };
      if (depth >= ETB_DEPTH_CAP || seen[id]) { capped = true; return { min: 0, max: 0 }; }
      var next = {}; next[id] = 1;
      for (var k in seen) if (seen.hasOwnProperty(k)) next[k] = 1;
      var results = e.possible_results || [];
      if (!results.length) return { min: 0, max: 0 };          // nothing planned after this one
      var mins = [], maxs = [];
      for (var i = 0; i < results.length; i++) {
        var r = results[i] || {};
        var targets = (r.next_experiment_ids || []).filter(function (t) { return !!exps[t]; });
        if (!targets.length) {
          // terminal, or an unplanned branch: this path ends here
          if (!r.terminal) sawUnplanned = true;
          mins.push(0); maxs.push(0);
          continue;
        }
        var bMin = null, bMax = 0;
        for (var j = 0; j < targets.length; j++) {
          var sub = walk(targets[j], depth + 1, next);
          var mv = 1 + sub.min, xv = 1 + sub.max;
          if (bMin === null || mv < bMin) bMin = mv;
          if (xv > bMax) bMax = xv;
        }
        mins.push(bMin === null ? 0 : bMin); maxs.push(bMax);
      }
      var mn = mins[0], mx = maxs[0];
      for (var q = 1; q < mins.length; q++) { if (mins[q] < mn) mn = mins[q]; if (maxs[q] > mx) mx = maxs[q]; }
      return { min: mn, max: mx };
    }
    var r0 = walk(expId, 0, {});
    return { min: r0.min, max: r0.max, unplanned: sawUnplanned, capped: capped };
  }
  ...
  /* This rebuilds every problem from an explicit field list, which means ANY field not named here is
     silently discarded on load — the doc saves correctly and comes back missing pieces. That cost a
     long hunt when FMEA evidence was added: the write was fine, the read threw it away. Anything
     added to a mode, effect or cause must be carried through here too. */
  function keepExperiments(src, out) {
    if (src && Array.isArray(src.experiments) && src.experiments.length) out.experiments = src.experiments;
    return out;
  }
  function migrateProblem(r) {
    ...
    return {
      ...
      modes: (r.modes || []).map(function (m) { m = m || {};
        return keepExperiments(m, { mid: m.mid || fmeaId('m'), mode: m.mode || '', status: m.status || 'open',
          effects: (m.effects || []).map(function (e) { e = e || {};
            return keepExperiments(e, { eid: e.eid || fmeaId('e'), effect: e.effect || '', status: e.status || 'open',
              causes: (e.causes || []).map(function (c) { c = c || {};
                return keepExperiments(c, { cid: c.cid || fmeaId('c'), cause: c.cause || '',
  ...
  // ---- exports --------------------------------------------------------------
  /* ---- Milestone pause / deprioritise ---------------------------------------------------------------
     Modelled the way objectives already end: a separate state record rather than a field on the
     milestone. The milestone itself is untouched, so everything that reads one keeps working, and the
     history of pausing survives instead of a field being overwritten and lost.
     'paused' means the date is suspect but the work is still intended; 'deprioritized' means it is not,
     but the record is worth keeping. They behave identically on the chart — the difference is for the
     person reading a review, not for the renderer.
     The LAST matching record wins, so resuming is appending {status:'active'} rather than hunting down
     and deleting the original: the pause stays in the history. */
  var MS_PAUSED = { paused: 1, deprioritized: 1 };
  function milestoneState(milestoneState_, msId){
    var arr = milestoneState_ || [], hit = null;
    for (var i = 0; i < arr.length; i++) {
      var r = arr[i];
      if (r && r.milestoneId === msId) hit = r;
    }
    return hit;
  }
  function milestonePaused(milestoneState_, msId){
    var r = milestoneState(milestoneState_, msId);
    return !!(r && MS_PAUSED[r.status]);
  }
  // every paused milestone id, for a count the UI can show without re-walking the list
  function pausedMilestoneIds(milestoneState_, milestones){
    var out = [];
    (milestones || []).forEach(function (m) {
      if (m && milestonePaused(milestoneState_, m.id)) out.push(m.id);
    });
    return out;
  }
  /* Appending rather than mutating: a resume is a new record, so the pause it supersedes is still on
     file. Callers pass the existing array and get a new one — no in-place edit of shared state. */
  function setMilestoneState(milestoneState_, msId, status, day, note){
    var arr = (milestoneState_ || []).slice();
    arr.push({ milestoneId: msId, status: status, day: (day == null ? null : day),
  ...
  var API = {
    ...
    quarterSpan: quarterSpan,
    quarterSpanImplied: quarterSpanImplied,
    quarterSpanRange: quarterSpanRange,
    ...
    milestoneState: milestoneState, milestonePaused: milestonePaused,
    pausedMilestoneIds: pausedMilestoneIds, setMilestoneState: setMilestoneState,
    ...
    etbTreesOf: etbTreesOf,
    etbTreeById: etbTreeById,
    etbActiveTree: etbActiveTree,
    nextStepRange: nextStepRange,
